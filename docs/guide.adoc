...
. <<layouts,Layout>> (Possible values: `tiles`, `accordion`, `stack`)
...
- `stack` full overlap: all members occupy one BSP leaf
...
A `stack` differs from an accordion because it exposes no padding or tab strip.
Every member gets exactly the same rectangle, and the focused member is in front.
Directional focus treats the whole stack as one BSP leaf; use `focus stack-prev`, `focus stack-next`, `focus stack-first`, `focus stack-last`, or `focus stack-recent` to select a member.
New windows opened while a stack member is focused join that stack.
...
`enable-bsp-layout` is not a normalization. When enabled, a new tiled window immediately splits the focused window's leaf.
The split axis follows the leaf's longest side, and the two children initially receive equal space.
Generic tree commands are still allowed; a repair pass only folds a temporary node with more than two children back to the binary invariant.
Accordion subtrees remain "escape islands", and a stack is treated as one BSP leaf.
`bsp-auto-balance = 'ancestors'` gives each affected subtree space proportional to its BSP-leaf count after an insertion, removal, stack, or directional mouse drop.
For example, splitting one side of a two-window layout changes the outer ratio from `1:1` to `1:2`, so all three slots receive equal virtual area while ratios inside unrelated subtrees remain untouched.
`bsp-auto-balance = 'workspace'` reapplies that rule recursively to every tiled subtree; `off` preserves split sizes exactly as before.
If an application rejects a tile smaller than its native minimum size, AeroSpace remembers the rejected dimension, expands that leaf, and keeps it inside the workspace.
At extreme depths, the expanded leaf can overlap a neighbor because the requested BSP geometry is physically too small for the application.
The former `enable-normalization-bsp-shape` key remains accepted as a deprecated alias so existing fork configs continue to work.
...
Dragging a tiled window never edits the tree while the mouse button is held.
A translucent preview follows the current target, and exactly one operation is committed when the button is released.
Releasing outside a valid target cancels the operation.

The middle 50% of the target uses `mouse-drop-action` (`swap` by default, or `stack` to combine both windows into one BSP leaf).
The former `mouse-drag-drop-action` key remains accepted as a deprecated alias; both of its legacy values map to `swap`.
The outer 25% on each edge is always a directional drop zone:

* Left or right places the dragged window beside the target in a horizontal split.
* Top or bottom places the dragged window beside the target in a vertical split.
* Dropping on an edge creates a two-window split for the target and dragged window; dropping the same pair into its existing two-window split only reorders it.

Mouse-created splits preserve the direction selected by the drop zone.
Dragging one member out of a stack through an edge extracts only that member.
Dropping beside a stack splits beside the whole stack; swapping with a stack exchanges whole BSP leaves.
