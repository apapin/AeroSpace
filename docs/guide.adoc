...
A third, opt-in normalization rewrites flat tiling subtrees into binary BSP shape:

[start=3]
. `enable-normalization-bsp-shape` (default `false`).
When enabled, every workspace refresh folds any `tiles` container that has more than two children into a focus-aware tree of two-child containers with alternating orientation.
This is the BSP / Fibonacci layout slot from the broader normalization framework discussed in https://github.com/nikitabobko/AeroSpace/issues/260[issue #260]: enabling it produces and preserves a Fibonacci-style tree on every refresh.
Accordion subtrees are skipped - they remain "escape islands" inside BSP workspaces.
...
By default, dragging a tiled window over another tiled window swaps the two windows without changing the container tree.

Set `mouse-drag-drop-action = 'reparent'` to make mouse dragging tree-aware.
The middle 50% of the target window keeps the existing swap behavior.
The outer 25% on each edge becomes a directional drop zone:
...
Tree normalizations still run after the drop.
Mouse-created splits preserve the direction selected by the drop zone, and `enable-normalization-bsp-shape` keeps the resulting tree binary.
