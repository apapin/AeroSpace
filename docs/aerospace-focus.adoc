...
(stack-prev|stack-next|stack-first|stack-last|stack-recent)::
Select a window inside the current `stack` leaf.
`stack-recent` returns to the previously focused member.
Stack focus targets don't support boundary or wrap-around options.
